...
async function verifyLinkedActiveSubject(
  ...
  if (col === 'office_staff' && !isActiveOfficeStaffStatus(data.status)) return false;
  return true;
}
...
async function fetchSelfProfileOfficeLines(
  ...
  linkedUserId: string,
  maxLines: number,
  hints?: { payrollLineRefs?: OfficeStaffPayrollLineRef[]; staffCode?: string },
): Promise<{ lines: OfficePayrollLine[]; error?: string }> {
  let staffCode = hints?.staffCode?.trim() || undefined;
  let payrollLineRefs = hints?.payrollLineRefs;

  if (!staffCode || payrollLineRefs === undefined) {
    try {
      ...
      if (staffSnap.exists()) {
        ...
        staffCode = staffCode || staffData.staffCode?.trim() || undefined;
        if (payrollLineRefs === undefined) payrollLineRefs = staffData.payrollLineRefs;
      }
    } catch {
      /* ignore */
    }
  ...
  const byLinkedUser = await fetchLinesBySubjectLinkedUserId(firestore, linkedUserId, { staffId });
  if (byLinkedUser.length > 0) {
    return {
      lines: byLinkedUser
  ...
  const friendlyApiErr =
    apiErr && /default credentials|credential/i.test(apiErr)
      ? 'ระบบยังไม่ได้ตั้งค่า Service Account สำหรับ sync สลิป — ให้ HR/Payroll กด «อัปเดต My Profile» ในงวดจ่าย'
      : apiErr;
...
async function syncSelfProfileOfficeLinesViaApi(
  staffId: string,
): Promise<{ lines: OfficePayrollLine[]; error?: string }> {
  const api = await fetchSelfPayrollLinesViaApi('office_staff', staffId);
  if (!api) return { lines: [], error: 'ไม่สามารถเชื่อมต่อ API ได้' };
  const lines = (api.lines as OfficePayrollLine[]) ?? [];
  const apiErr = api.error;
  const friendlyApiErr =
    apiErr && /default credentials|credential/i.test(apiErr)
      ? 'ระบบยังไม่ได้ตั้งค่า Service Account สำหรับ sync สลิป — ให้ HR/Payroll กด «อัปเดต My Profile» ในงวดจ่าย'
      : apiErr;
  return { lines, error: lines.length ? undefined : friendlyApiErr };
}
...
export async function fetchOfficeStaffPayrollLines(
  ...
  if (options?.linkedUserId) {
    ...
    const result = await fetchSelfProfileOfficeLines(firestore, staffId, options.linkedUserId, maxLines, {
      payrollLineRefs: options.payrollLineRefs,
...
export function useOfficeStaffPayrollLines(
  ...
  options?: {
    ...
    payrollLineRefs?: OfficeStaffPayrollLineRef[];
    staffCode?: string;
    /** เมื่อโหลดครั้งแรกแล้วว่าง — ลอง sync ผ่าน Admin API อัตโนมัติ 1 ครั้ง */
    autoSyncWhenEmpty?: boolean;
  },
) {
  const linkedUserId = options?.linkedUserId ?? null;
  const payrollLineRefs = options?.payrollLineRefs;
  const staffCode = options?.staffCode;
  const autoSyncWhenEmpty = options?.autoSyncWhenEmpty ?? false;
  ...
  const loadLines = useCallback(
    async (tryApiSync: boolean): Promise<{ lines: OfficePayrollLine[]; error?: string }> => {
      if (!firestore || !staffId?.trim() || !enabled || !linkedUserId) {
        return { lines: [] };
      }
      const ok = await verifyLinkedActiveSubject(firestore, 'office_staff', staffId, linkedUserId);
      if (!ok) {
        return { lines: [], error: 'ไม่พบทะเบียนที่ผูกบัญชี หรือสถานะไม่ใช่ ACTIVE — ติดต่อ HR' };
      }

      const hints = { payrollLineRefs, staffCode };
      let result = await fetchSelfProfileOfficeLines(firestore, staffId, linkedUserId, 100, hints);
      if (result.lines.length === 0 && tryApiSync) {
        setSyncBusy(true);
        const api = await syncSelfProfileOfficeLinesViaApi(staffId);
        setSyncBusy(false);
        if (api.lines.length > 0) {
          result = { lines: api.lines };
        } else {
          const retry = await fetchSelfProfileOfficeLines(firestore, staffId, linkedUserId, 100, hints);
          if (retry.lines.length > 0) {
            result = retry;
          } else if (api.error) {
            result = { ...result, error: api.error };
          }
        }
      }
      return result;
    },
    [firestore, staffId, enabled, linkedUserId, payrollLineRefs, staffCode],
  );
  ...
  const reload = useCallback(async () => {
    ...
    try {
      if (linkedUserId) {
        const result = await loadLines(true);
  ...
  }, [firestore, staffId, enabled, linkedUserId, loadLines]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        if (linkedUserId) {
          const result = await loadLines(autoSyncWhenEmpty);
          if (cancelled) return;
          setLines(result.lines);
          if (result.lines.length === 0) {
            setSyncHint(
  ...
  }, [firestore, staffId, enabled, linkedUserId, loadLines, autoSyncWhenEmpty]);

  return { lines, isLoading: isLoading || syncBusy, error, syncHint, reload };
}
