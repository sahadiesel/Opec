...
export function validateWorkerDocumentPhotoFile(file: File): string | null {
...
/** อัปโหลดรูปเอกสาร — บีบให้ไม่เกิน 500 KB */
export async function uploadWorkerDocumentPhoto(
  ...
  const err = validateWorkerDocumentPhotoFile(file);
  ...
  const safeType = safeFileSegment(documentType || 'doc');
  const safeName = safeFileSegment(file.name.replace(/\.[^.]+$/, '') + '.jpg');
  const path = `worker_document_photos/${workerId}/${safeType}/${Date.now()}_${id.slice(0, 8)}_${safeName}`;
