import type { FirebaseApp } from 'firebase/app';
import { getStorage, ref, uploadBytes, getDownloadURL } from 'firebase/storage';
import type { WaveMonthTimesheetPhotoAttachment } from '@/lib/types';
import { compressImageFileToMaxSize } from '@/lib/storage/image-compress';

const IMAGE_TYPES = new Set(['image/jpeg', 'image/png', 'image/webp', 'image/heic', 'image/heif']);
const MAX_IMAGE_COMPRESSED = 500 * 1024;
const MAX_IMAGE_ORIGINAL_BYTES = 25 * 1024 * 1024;
export const MAX_PDF_BYTES = 10 * 1024 * 1024;

function safeFileSegment(name: string): string {
  return name.replace(/[^\w.\-()\u0E00-\u0E7F]+/g, '_').slice(0, 80) || 'file';
}

export function isPdfFile(file: File): boolean {
  const t = (file.type || '').toLowerCase();
  if (t === 'application/pdf') return true;
  return file.name.toLowerCase().endsWith('.pdf');
}

export function isPdfAttachment(meta?: { contentType?: string; fileName?: string; downloadUrl?: string }): boolean {
  const ct = (meta?.contentType || '').toLowerCase();
  if (ct.includes('pdf')) return true;
  const name = (meta?.fileName || meta?.downloadUrl || '').toLowerCase();
  return name.includes('.pdf');
}

export function validateWorkerCredentialAttachmentFile(file: File): string | null {
  if (isPdfFile(file)) {
    if (file.size > MAX_PDF_BYTES) {
      return 'ไฟล์ PDF ใหญ่เกินไป (จำกัด 10 MB)';
    }
    return null;
  }
  if (!file.type.startsWith('image/') && !IMAGE_TYPES.has(file.type)) {
    return 'รองรับเฉพาะรูปภาพ (JPEG, PNG, WebP) หรือ PDF';
  }
  if (file.size > MAX_IMAGE_ORIGINAL_BYTES) {
    return 'ไฟล์รูปต้นฉบับใหญ่เกินไป (จำกัด ~25 MB ก่อนบีบอัด)';
  }
  return null;
}

export async function uploadWorkerCredentialAttachment(
  firebaseApp: FirebaseApp,
  storageFolder: string,
  workerId: string,
  itemCode: string,
  file: File,
): Promise<WaveMonthTimesheetPhotoAttachment> {
  const err = validateWorkerCredentialAttachmentFile(file);
  if (err) throw new Error(err);

  const id =
    typeof crypto !== 'undefined' && crypto.randomUUID
      ? crypto.randomUUID()
      : `${Date.now()}_${Math.random().toString(36).slice(2, 11)}`;

  const safeCode = safeFileSegment(itemCode || 'item');
  let blob: Blob = file;
  let contentType = file.type || 'application/octet-stream';
  let safeName: string;

  if (isPdfFile(file)) {
    contentType = 'application/pdf';
    const base = file.name.replace(/\.[^.]+$/i, '') || 'document';
    safeName = safeFileSegment(`${base}.pdf`);
  } else {
    blob = await compressImageFileToMaxSize(file, MAX_IMAGE_COMPRESSED);
    contentType = blob.type || 'image/jpeg';
    safeName = safeFileSegment(file.name.replace(/\.[^.]+$/, '') + '.jpg');
  }

  const path = `${storageFolder}/${workerId}/${safeCode}/${Date.now()}_${id.slice(0, 8)}_${safeName}`;
  const storage = getStorage(firebaseApp);
  const r = ref(storage, path);
  await uploadBytes(r, blob, { contentType });
  const downloadUrl = await getDownloadURL(r);

  return {
    id,
    storagePath: path,
    downloadUrl,
    fileName: file.name,
    contentType,
    uploadedAt: Date.now(),
  };
}
