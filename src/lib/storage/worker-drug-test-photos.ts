...
export function validateWorkerDrugTestPhotoFile(file: File): string | null {
...
/** อัปโหลดรูปผลตรวจสารเสพติด — บีบให้ไม่เกิน 500 KB */
export async function uploadWorkerDrugTestPhoto(
  ...
  const err = validateWorkerDrugTestPhotoFile(file);
  ...
  const path = `worker_drug_test_photos/${workerId}/${substanceKey}/${Date.now()}_${id.slice(0, 8)}_${safeName}`;
