...
export default function StoreItemsPage() {
  ...
  const runEquipmentPrint = useCallback(
    async (scope: 'filtered' | 'all') => {
        ...
        toast({
          ...
          description:
            scope === 'filtered'
              ...
              : 'ยังไม่มีรายการอุปกรณ์ (ไม่รวม PPE) ในระบบ',
        });
        return;
      }
      ...
      try {
        ...
        const scopeTitle =
          scope === 'filtered' ? 'พิมพ์ตามตัวกรองปัจจุบัน' : 'พิมพ์ทั้งหมด (ไม่รวม PPE)';
        ...
        const ok = await openStandardPrintWindow({
          windowTitle: 'Store-Equipment-Registry',
          suggestedFileName: `Store-Equipment-${scope === 'filtered' ? 'Filtered' : 'All'}`,
          bodyInnerHtml: body,
          htmlLang: 'th',
        });
      ...
      <div className="space-y-6 max-w-[1600px] mx-auto">
        ...
        <Dialog open={printDialogOpen} onOpenChange={setPrintDialogOpen}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>พิมพ์รายการทะเบียนอุปกรณ์</DialogTitle>
              <DialogDescription>
                พิมพ์ตามตัวกรองปัจจุบัน หรือทั้งทะเบียนอุปกรณ์ (ไม่รวม PPE) — รวมรุ่นย่อยทุกเมน
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-3 text-sm">
              ...
              <p className="text-xs text-muted-foreground">ทั้งทะเบียน: {allPrintRowCount} แถว (รวมรุ่นย่อย)</p>
            </div>
            <DialogFooter className="flex-col sm:flex-row gap-2">
              <Button
                ...
                onClick={() => void runEquipmentPrint('filtered')}
              >
              ...
              <Button
                ...
                onClick={() => void runEquipmentPrint('all')}
              >
                {printBusy ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Printer className="h-4 w-4 mr-2" />}
                  ...
                  {filteredDisplayRows.map((row) =>
                    row.kind === 'standalone' ? (
                      <TableRow key={row.item.id} className="hover:bg-muted/30 transition-colors">
                        <TableCell className={`${sc.codeCell} text-primary`}>{row.item.itemCode}</TableCell>
                        ...
                        <TableCell className={sc.categoryCell}>{renderEquipmentCategoryCell(row.item)}</TableCell>
                      ...
                      <Fragment key={row.header.id}>
                        <TableRow className="bg-muted/50 hover:bg-muted/60 border-t-2 border-t-primary/20">
                          <TableCell className={`${sc.codeCell} text-primary`}>{row.header.itemCode}</TableCell>
                          <TableCell className={`${sc.nameCell} text-primary`}>
                          ...
                          <TableCell className={sc.categoryCell}>{renderEquipmentCategoryCell(row.header)}</TableCell>
                          <TableCell className={sc.stockCell}>
                        ...
                        {isVariantExpanded(row.header.id)
                          ...
                          <TableRow key={child.id} className="hover:bg-muted/20 border-l-4 border-l-primary/25">
                            ...
                            <TableCell className={sc.categoryCell}>{renderEquipmentCategoryCell(child)}</TableCell>
                            <TableCell className={sc.stockCell}>
