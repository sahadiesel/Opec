...
import { canMarkPurchaseVendorBillPaid, canView, isSystemAdmin } from '@/lib/permissions';
...
export function PurchaseVendorBillsList({ mode }: { mode: PurchaseVendorBillsListMode }) {
  ...
  const detailBasePath = mode === 'store' ? '/store/vendor-bills' : '/ap-bills/vendor-bills';
  ...
  const okAccounting = useMemo(
    () =>
      !!currentUser &&
      (canView(currentUser, 'ap_bills') ||
        canView(currentUser, 'accounts_payable') ||
        canMarkPurchaseVendorBillPaid(currentUser)),
    [currentUser],
  );

  const ok = mode === 'store' ? okStore : okAccounting;

  const showAdminDelete = useMemo(
    () => !!currentUser && (isSystemAdmin(currentUser) || isSimpleAdmin(currentUser)),
    [currentUser],
  );
  const showBillDeleteColumn = showAdminDelete || (mode === 'store' && okStore);
  ...
  const purchasesQuery = useMemoFirebase(() => {
    if (!firestore || !okStore || mode !== 'store') return null;
    return query(collection(firestore, 'purchases'), where('status', 'in', ['APPROVED', 'ISSUED', 'COMPLETED']));
  }, [firestore, okStore, mode]);
    ...
    return (
      <AppShell user={currentUser as User} onLogout={() => {}}>
        <div className="max-w-3xl mx-auto py-16 text-center text-muted-foreground">
          คุณไม่มีสิทธิ์ดูรายการใบรับวางบิลจากใบสั่งซื้อ
        </div>
      </AppShell>
    );
  }
          ...
          <div className="flex flex-wrap gap-2">
            {isStoreMode ? (
              ...
              <Button variant="outline" size="sm" asChild>
                <Link href="/store/vendor-bills">เปิดมุมมองคลัง (สร้างใบร่าง)</Link>
              </Button>
            ) : null}
          </div>
        </div>
        ...
        <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && !isDeleting && setDeleteTarget(null)}>
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              ...
              <Button type="button" variant="destructive" disabled={isDeleting} onClick={() => void handleConfirmDeleteBill()}>
